...
# PR_condensed <- read.csv("raw-data/PRcondensed.csv") %>%
#   clean_names()
...
#   
# parse_character(x1, locale = locale(encoding = "UTF8"))
# parse_character(x2, locale = locale(encoding = "UTF8"))
# parse_character(x3, locale = locale(encoding = "UTF8"))
# parse_character(x4, locale = locale(encoding = "UTF8"))
# parse_character(x5, locale = locale(encoding = "UTF8"))
# parse_character(x6, locale = locale(encoding = "UTF8"))
# parse_character(x7, locale = locale(encoding = "UTF8"))
# parse_character(x8, locale = locale(encoding = "UTF8"))
# parse_character(x9, locale = locale(encoding = "UTF8"))
# parse_character(x10, locale = locale(encoding = "UTF8"))
# parse_character(x11, locale = locale(encoding = "UTF8"))
# parse_character(x12, locale = locale(encoding = "UTF8"))
# 
party <- c(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12)

party2 <- as.data.frame(party)
# 
# write_csv(party2, "raw-data/party.csv")
...
# PR_englishnamechanged <- PR_englishnamechanged %>%
#   mutate(cons_or_not = ifelse(
#     englishpartyname == "democracyjustice", 1, 0)%>%
#       ifelse(englishpartyname == "newdemocracy", 1, .) %>%
#       ifelse(englishpartyname == "democracyfreedom", 1, .) %>%
#       ifelse(englishpartyname == "unifiedpeople", 1, .) %>%
#       ifelse(englishpartyname == "newkorea", 1, .) %>%
#       ifelse(englishpartyname == "freedomdemocracyteam", 1, .) %>%
#       ifelse(englishpartyname == "onenation", 1, .) %>%
#       ifelse(englishpartyname == "saenuri", 1, .) %>%
#       ifelse(englishpartyname == "freedomadvance", 1, .) %>%
#       ifelse(englishpartyname == "propark", 1, .) %>%
#       ifelse(englishpartyname == "creativekorea", 1, .)
#   )
...
# PR_condensed$cons_or_not <- ifelse(
#   (PR_condensed$partyname == x1) |
#     (PR_condensed$partyname == x2) |
#     (PR_condensed$partyname == x3) |
#     (PR_condensed$partyname == x4) |
#     (PR_condensed$partyname == x5) |
#     (PR_condensed$partyname == x6) |
#     (PR_condensed$partyname == x7) |
#     (PR_condensed$partyname == x8) |
#     (PR_condensed$partyname == x9) |
#     (PR_condensed$partyname == x10) |
#     (PR_condensed$partyname == x11) |
#     (PR_condensed$partyname == x12)
#     , 1, 0)
...
# some kind of regression model
#coefficient model or coefficient plot
# have some model where you have outcome of some variable and couple of controls, if it's linear model and have binary code 
#look at data and look at it, have a separate page
# about page (yourself), what kinds of questions ou want to answer, some type of interactive graph, separate tab for model page where you do some statistics and justify why you picked that particular model
# present what they have, based off feedback, amend graph and add extra explanations
...
# save(PR_twopartyvoteshare, file = "Korean_Parliament_Analysis/data_files/PR_twopartyvoteshare.Rdata")
